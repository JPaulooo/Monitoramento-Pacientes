...
def carregar_dados():
    file_id = '1dPoWECBeH3h4IfMIi-1-A-EVZui6oB1I'
...
if nr_selecionado == "🌐 Todos os Pacientes":
    ...
    evento_clique = st.dataframe(
        ...
        column_config={
            "NR": st.column_config.TextColumn("Nr Atendimento", width=15),
            "DATA_REFERENCIA": st.column_config.DatetimeColumn("Última Avaliação", format="DD/MM/YYYY HH:mm", width=15),
            "Risco_Max": st.column_config.ProgressColumn("Maior Risco IA (%)", format="%.1f", min_value=0, max_value=100, width=500),
            "Horizonte": st.column_config.TextColumn("Horizonte Crítico", width=15),
            "NEWS_Status": st.column_config.TextColumn("Mediana NEWS", width=15),
            "IndiceChoque": st.column_config.NumberColumn("Índice Choque", format="%.2f", width=15),
            "IndiceRox": st.column_config.NumberColumn("Índice ROX", format="%.2f", width=15),
            "UltimoSPO2": st.column_config.NumberColumn("SpO2 (%)"),
            "UltimoFR": st.column_config.TextColumn("FR (rpm)", width=15),
            "DeltaFR6h": st.column_config.TextColumn("DeltaFR", width=15),
            "UltimoFC": st.column_config.TextColumn("FC (bpm)", width=15),
            "DeltaFC6h": st.column_config.TextColumn("DeltaFC", width=15),
            "UltimoPA": st.column_config.TextColumn("PA Sistólica", width=15),
            "DeltaPA6h": st.column_config.TextColumn("DeltaPA", width=15)
        },
        hide_index=True,
